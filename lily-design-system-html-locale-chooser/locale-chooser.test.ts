...
function button(): HTMLButtonElement {
    return document.body.querySelector<HTMLButtonElement>(".locale-chooser-button")!;
}

function list(): HTMLUListElement {
    return document.body.querySelector<HTMLUListElement>(".locale-chooser-list")!;
}

function options(): HTMLLIElement[] {
    return [...document.body.querySelectorAll<HTMLLIElement>(".locale-chooser-option")];
}
...
describe("<locale-chooser> — pure helpers (§7.2)", () => {
...
describe("<locale-chooser> — markup contract (§7.1)", () => {
    ...
    test("§7.1 the button renders the globe glyph, hidden from assistive tech", async () => {
        ...
        // monochrome, matching theme-chooser's ◑ rather than the blue
        // colour-emoji globe.
...
describe("<locale-chooser> — keyboard contract (APG listbox, §7.24–§7.28)", () => {
...
describe("<locale-chooser> — locale application (§7.3)", () => {
...
describe("<locale-chooser> — initial-value resolution (§7.4)", () => {
...
describe("<locale-chooser> — property API (§7.5)", () => {
    test("§7.22 setting el.locales as an array mirrors the CSV attribute", async () => {
        const el = mount({ label: "Language", locales: "en,fr" }) as LocaleChooser;
        await flush();
        el.locales = ["en", "fr", "ar"];
        await flush();
    ...
    test("§7.23 the consumer class is appended to the root class hook", async () => {
        ...
        await flush();
        const root = document.body.querySelector("div.locale-chooser")!;
        expect(root.className).toBe("locale-chooser my-picker");
    });
...
describe("<locale-chooser> — custom rendering by subclass (§7.29)", () => {
    ...
    test("§7.29 renderButtonContent replaces the glyph and keeps the aria wiring", async () => {
        ...
        expect(custom.closest("button")?.className).toBe("locale-chooser-button");
