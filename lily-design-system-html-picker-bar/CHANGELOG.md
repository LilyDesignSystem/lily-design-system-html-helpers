...
**Package renamed: `lily-design-system-html-picker-bar` → `@lilydesignsystem/html-picker-bar`.** npm scoped packages
...
relative to `lily-design-system-html-picker-bar`'s last published version (`0.1.0`).
This is this package's first dedicated `CHANGELOG.md`; its prior
history (as `lily-design-system-html-picker-bar`) is recorded in the root
