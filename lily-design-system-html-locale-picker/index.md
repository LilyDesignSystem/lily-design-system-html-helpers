...
without rewriting the picker, and it lets the picker stay headless:
...
On every locale change the picker performs four steps, in order:

1. **Resolve the target.** Default is `document.documentElement`
   (the `<html>` tag); override with `el.target` for region-scoped
   localisation.
2. **Write `lang`.** The picker normalises the consumer-form code
...
6. `locales[0]`
7. `""` — nothing is applied; the picker waits for user interaction
...
| `class`                  | string         | no       | Extra class on the `<fieldset>`.     |
...
picker.locales = ["en", "fr", "ar"];               // CSV-encoded in attribute
picker.localeLabels = { en: "English", fr: "Français", ar: "العربية" };
picker.detectFromNavigator = true;                 // mirrors boolean attribute
picker.applyDir = false;                           // → apply-dir="false"
...
the customisation surface is **subclassing**. Extend `LocalePicker`
...
a fresh mount the picker reads back the stored code as part of the
...
- The rendered root is a `<fieldset>` with `role="radiogroup"` and
  `aria-label={label}`.
- Native `<input type="radio">` gives Arrow / Space / Tab semantics
  for free.
- Each option carries `lang="…"` so its name is pronounced in the
  right language (WCAG 3.1.2 Language of Parts).
...
the matching `<locale-picker value="…">` host.
