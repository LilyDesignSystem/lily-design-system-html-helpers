...
browser; there is no "picker on the server". The static-site
...
the picker's `value` attribute. The client then upgrades into a
...
## What the picker emits before JS upgrade
...
No `<fieldset>`, no radios, no `lang` / `dir` writes — those all
appear on hydration.
...
2. Renders the `<fieldset>` and `<input type="radio">` children.
...
Pass the resolved value as the `value` attribute so the picker
...
├── _includes/
│   └── layout.njk            ← inlines <html lang> + picker value
...
on the wire. The picker mounts already showing the right radio
checked, no layout shift.

For per-request locale (cookies) on Eleventy, use a small client-
side `<script>` early in `<head>` that reads `document.cookie` and
writes `<html lang>` before the picker upgrades; or migrate to
...
server-rendered layout into the picker's `value` attribute:
...
client-side script that reads the cookie before the picker
upgrades.
...
storage and sets `<html lang>` before the picker mounts:
...
first paint already has the right `lang` / `dir`. The picker's own
...
The picker's `localechange` handler then navigates to the new
prefix:
...
The "cookie-resolved attribute" is just the `value=` on the picker
plus the matching `lang=` on `<html>`. The browser:

1. Receives `<html lang="fr" dir="ltr">` — first paint is correct.
2. Parses `<locale-picker value="fr">` — the picker constructor
...
+ pre-seed pattern; streaming doesn't change the picker's
behaviour.
...
server-side preference store. The picker stays transport-agnostic
