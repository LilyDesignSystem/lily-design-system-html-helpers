...
| APG Radio Group pattern | Single-selection radio group with one `tabindex=0` representative and roving tabindex elsewhere — provided by the platform. |
...
The same logic applies when you subclass to render a `<select>`.
...
customElements.define("viewer-language-locale-picker", ViewerLanguageLocalePicker);
```
...
A subclass can render a `<select>` instead of radios. Native
