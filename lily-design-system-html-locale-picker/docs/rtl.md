...
By default `apply-dir` is "absent or true" — the picker writes both
...
picker.applyDir = false;  // setAttribute("apply-dir", "false")
```
...
The picker keeps `lang` accurate; the selectors do the rest.
...
- You manage `dir` server-side and don't want the picker to clobber
  it on hydration.
...
2. **DevTools** — toggle `dir="rtl"` on `<html>` directly in the
   Elements panel. Compare against the picker-controlled state.
3. **Automated** — add a Playwright spec that flips the picker and
