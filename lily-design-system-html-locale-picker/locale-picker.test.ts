...
describe("<locale-picker> — markup contract (§7.1)", () => {
    ...
    test("§7.2 aria-label is the supplied label", async () => {
        ...
        await flush();
        const fs = document.body.querySelector("fieldset")!;
        expect(fs.getAttribute("aria-label")).toBe("Choose language");
    });

    test("§7.3 one radio per locale, sharing the supplied name", async () => {
    ...
    test("§7.4 each radio carries the locale code as its value", async () => {
        mount({ label: "Language", locales: LOCALES.join(",") });
        await flush();
        const radios = document.body.querySelectorAll<HTMLInputElement>("input[type='radio']");
    ...
    test("§7.5 each option carries lang in BCP 47 hyphen form", async () => {
        ...
        await flush();
        const labels = document.body.querySelectorAll<HTMLElement>(".locale-picker-option");
...
describe("<locale-picker> — locale application (§7.3)", () => {
    ...
    test("§7.16 selecting a different radio updates lang, dir, and fires localechange (consumer form)", async () => {
        ...
        await flush();
        const radios = document.body.querySelectorAll<HTMLInputElement>("input[type='radio']");
        radios[1].click(); // en_US (consumer form)
        await flush();
        expect(document.documentElement.lang).toBe("en-US");
