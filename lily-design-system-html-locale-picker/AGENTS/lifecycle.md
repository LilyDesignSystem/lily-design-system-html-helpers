...
user clicks a radio
  │
  ▼
input.change → setter writes el.value = newCode
  │
  ▼
...
value="…">` host. The picker upgrades without changing anything
visible. See [`./ssr.md`](./ssr.md).

## Unmount

The element does not clean up `lang` / `dir` on unmount. The picker
...
attribute is author-managed once written — neither the picker nor
...
picker.addEventListener("disconnectedreset", () => {
