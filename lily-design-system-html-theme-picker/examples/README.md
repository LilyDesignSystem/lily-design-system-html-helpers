...
`lily-design-system-html-theme-chooser`. Each file is a runnable
...
- A directory of theme CSS files served at `/assets/themes/`
  ...
  themes.
- A built copy of the `theme-chooser` ES module at
  `/dist/theme-chooser.js` (or any path you prefer; adjust the
...
| #  | File                                        | Demonstrates                              |
|----|---------------------------------------------|-------------------------------------------|
| 1  | [`01-basic.html`](./01-basic.html)          | Minimal three-theme chooser.               |
...
positioning: the `<ul class="theme-chooser-list">` renders in normal
...
2. Place a built copy of `theme-chooser.js` at the path the example
   references (typically `/dist/theme-chooser.js`).
