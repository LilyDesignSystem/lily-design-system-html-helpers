...
picker's first `connectedCallback` sees `value="dark"` and skips
...
browser styles, then on hydration the picker sets `data-theme="dark"`
...
`value` attribute on the `<theme-picker>` host so the picker doesn't
...
matching `<link>`. The picker upgrades on hydration, reads back
...
The cookie-resolved value arrives in the HTML response; the picker
...
## When the picker has no SSR alternative
...
the un-upgraded `<theme-picker>` host (empty), then the picker
upgrades and the rendered fieldset appears. The visible change is
the appearance of the radios, not a re-paint of the rest of the page
— because the inlined `<html data-theme>` is unchanged.
...
the picker will read (cookie + server-side resolution).

## Why we don't auto-resolve from the cookie

The picker is transport-agnostic. Cookies are right for dynamic SSR
(Astro, Node + Express), but build-time SSG (Eleventy, Hugo) has no
request context. The picker accepts the resolved value via the
`value` attribute and lets the host wire the integration.

## Service-worker / offline

The picker plays well with service-worker offline caches: every URL
...
theme catalog and the picker switches instantly even offline.
