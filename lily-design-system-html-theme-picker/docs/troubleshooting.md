...
## "Element upgrades but stays empty (no fieldset)"

**Likely cause.** Required attributes (`label`, `themes-url`,
`themes`) are missing. The picker doesn't throw — it just renders
no options.
...
`el.target` JS property), or designate one picker as the "global"
...
- Confirm the surrounding code isn't removing and re-adding the
  picker every render (e.g. inside a framework component whose
...
picker.value = "dark" satisfies Slug;
```
...
## "Inside a `<dialog>` the picker forgets its selection on close"

`<dialog>` doesn't detach its contents from the DOM on `close()`,
so the picker stays mounted and its `disconnectedCallback` does not
...
`dialog.remove()`); change to `dialog.close()` to keep the picker
mounted.
...
- Confirm you're listening on the picker, on an ancestor (events
...
— if it does, the duplicate import is the bug, not the picker.
