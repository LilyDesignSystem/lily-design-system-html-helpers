...
No `<fieldset>`, no radios, no `<link>` in `<head>` — those all
appear on hydration.
...
2. Renders the `<fieldset>` and radios as children.
...
resolved value as the `value` attribute on the host so the picker
...
4. On the client, the picker reads `localStorage` and may apply a
...
client-side script that reads `localStorage` before the picker
...
`<html data-theme>` before the picker mounts:
...
- For static sites (`output: "static"`), cookies aren't available at
  ...
  upgrade.
- `client:load` is fine for the picker; the upgrade timing doesn't
...
picker handles this by applying the stored choice in
...
the same value the picker will read (cookie + server-side resolution).
