...
`aria-label` on the rendered `<fieldset role="radiogroup">`. Always
...
Comma-separated list of theme slugs the picker exposes as options.
Each slug is used both as the radio `value` and as the URL path
...
```ts
(picker as ThemePicker).themes = ["light", "dark", "abyss"];
// equivalent to: picker.setAttribute("themes", "light,dark,abyss")
```

The CSV form makes the picker safe to declare in HTML without
escaping commas.
...
```ts
picker.setAttribute("value", "dark");          // → applies "dark"
picker.value = "abyss";                         // → applies "abyss"
picker.getAttribute("value");                   // → "abyss"
```

When supplied as a non-empty string on first mount, the picker
...
```ts
(picker as ThemePicker).themeLabels = {
    light: "Bright",
    dark: "Midnight",
};
// equivalent to:
// picker.setAttribute("theme-labels", '{"light":"Bright","dark":"Midnight"}')
```
...
Extra CSS class on the rendered `<fieldset>`. Always emitted after
...
(picker as ThemePicker).target = section;
picker.value = "dark"; // section now has data-theme="dark"
```
...
rendered `<fieldset>`. This is the natural behaviour of custom
elements: the host is where the consumer declares them; the
rendered children are recreated on every render.

If you need an attribute to land on the rendered fieldset, write a
