...
```ts
// One side-effect import registers <theme-picker> globally:
...
`customElements.get("theme-picker")` so re-imports and SSR contexts
don't throw.
...
2. Place the custom element in your markup. It renders a
   `<fieldset>` with one radio per theme on `connectedCallback`.
...
When the user picks `dark`, the element:
...
| --------------- | -------------- | -------- | ------------------------------------------ |
| `label`         | string         | yes      | `aria-label` on the rendered radiogroup.   |
...
| `class`         | string         | no       | Extra class on the rendered `<fieldset>`.  |
...
picker.themeLabels = { light: "Bright", dark: "Midnight" };
```
...
| --------------- | ------------------- | ------- | -------- | ----------------------------------------------------- |
| `themechange`   | `{ theme: string }` | yes     | yes      | After the picker applies a new theme.                 |
...
DOM is touched; the SSG emits the literal `<theme-picker>` tag, and
...
the picker doesn't re-resolve from storage and clobber the inlined
attribute.
...
Pass a distinct `name` attribute to each picker. The `name` is used
...
Example: [`examples/03-multiple-pickers.html`](./examples/03-multiple-pickers.html).
