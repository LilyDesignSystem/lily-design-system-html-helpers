...
export type ThemeSelectChangeDetail = {
    theme: string;
};
 ...
 *
 * Mirrors `localeName` in locale-select. The element's `labelFor`
 ...
 * Mirrors `matchNavigatorLanguage` in locale-select.
 *
...
/** Custom-element class implementing `<theme-select>`. */
export class ThemeSelect extends HTMLElement {
    ...
    readonly #baseId = nextThemeSelectId();
     ...
     * visit. Mirrors `detectFromNavigator` in locale-select, including
    ...
    disconnectedCallback(): void {
        ...
        const sameName = document.querySelectorAll(
            `theme-select[name="${this.name}"]`,
        );
        if (sameName.length === 0) {
            const link = document.head.querySelector(
                `link[data-lily-theme-select="${this.name}"]`,
            );
            link?.remove();
        }
    }
    ...
    #applyTheme(slug: string): void {
        ...
        this.dispatchEvent(
            new CustomEvent<ThemeSelectChangeDetail>("themechange", {
                detail: { theme: slug },
                bubbles: true,
                composed: true,
            }),
    ...
    #render(): void {
        ...
        root.className = `theme-select ${extraClass}`.trim();
