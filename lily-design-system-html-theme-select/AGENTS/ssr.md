...
`<theme-select>` tag with attributes; the browser runtime
constructs the element and runs `connectedCallback` after parsing
the HTML.

If the consumer pre-renders `<theme-select value="dark">`, the
...
`value` attribute on the `<theme-select>` host so the select doesn't
...
<html lang="en" data-theme="{{ theme.defaultTheme }}">
    <head>
        ...
        <script type="module" src="/dist/lily-design-system-html-theme-select.js"></script>
    </head>
    <body>
        {{ content | safe }}
        <theme-select
