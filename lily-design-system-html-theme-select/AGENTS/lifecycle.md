...
```
parser sees <theme-select label="…" themes-url="…" themes="…">
  │
  ▼
...
The class definition file (`theme-select.ts`) has no top-level DOM
...
`<theme-select>` host. The select upgrades without changing
anything visible. See [`./ssr.md`](./ssr.md).

## Unmount

`disconnectedCallback` removes the managed `<link>` only when no
other `<theme-select>` with the same `name` remains in the
...
document.querySelector("theme-select")?.remove();
```
