...
import {
    ...
    type ShareChooserShareDetail,
    type ShareChooserUrlDetail,
    type ShareTarget,
...
function trigger(): HTMLButtonElement {
    return document.body.querySelector<HTMLButtonElement>(".share-chooser-button")!;
}

function list(): HTMLUListElement {
    return document.body.querySelector<HTMLUListElement>(".share-chooser-list")!;
}

function links(): HTMLAnchorElement[] {
    return [...document.body.querySelectorAll<HTMLAnchorElement>(".share-chooser-target")];
}

function copyItem(): HTMLButtonElement | null {
    return document.body.querySelector<HTMLButtonElement>(".share-chooser-copy");
}

function statusRegion(): HTMLParagraphElement {
    return document.body.querySelector<HTMLParagraphElement>(".share-chooser-status")!;
}

function items(): HTMLElement[] {
    return [
        ...list().querySelectorAll<HTMLElement>(".share-chooser-target, .share-chooser-copy"),
    ];
}
...
describe("<share-chooser> — pure helpers", () => {
    test("nextShareChooserId mints unique, prefixed ids", () => {
        const a = nextShareChooserId();
        const b = nextShareChooserId();
        expect(a).toMatch(/^share-chooser-\d+$/);
        expect(b).toMatch(/^share-chooser-\d+$/);
        expect(a).not.toBe(b);
    });
...
describe("<share-chooser> — markup contract (§7.1–§7.6)", () => {
    test("§7.1 renders a disclosure button controlling a list", async () => {
        mount({ label: "Share", url: URL_UNDER_TEST });
        await flush();
        const root = document.body.querySelector("div.share-chooser")!;
    ...
    test("§7.1 the trigger class is share-chooser-button", async () => {
        mount({ label: "Share", url: URL_UNDER_TEST });
        await flush();
        // Follows the catalog's `{helper}-button` convention, same as
        // theme-chooser / locale-chooser / text-size-chooser.
        expect(document.body.querySelector(".share-chooser-button")).not.toBeNull();
        expect(document.body.querySelector(".share-chooser-trigger")).toBeNull();
    });
    ...
    test("§7.3 destinations are real links, not role=menuitem", async () => {
        ...
        for (const a of all) {
            ...
            expect(a.closest("li")?.className).toBe("share-chooser-list-item");
        }
    });
...
describe("<share-chooser> — copy to clipboard (§7.7–§7.10)", () => {
    ...
    test("§7.7 copying also dispatches a bubbling `copy` CustomEvent", async () => {
        const clip = stubClipboard();
        const seen: ShareChooserUrlDetail[] = [];
        document.body.addEventListener("copy", (e) => {
            seen.push((e as CustomEvent<ShareChooserUrlDetail>).detail);
        });
        mount({ label: "Share", url: URL_UNDER_TEST, "copy-label": "Copy link" });
        await flush();
        click(trigger());
...
describe("<share-chooser> — native share sheet (§7.11–§7.14)", () => {
...
describe("<share-chooser> — keyboard and dismissal (§7.15–§7.19)", () => {
    async function openList(): Promise<ShareChooser> {
        const el = mount({
            label: "Share",
            url: URL_UNDER_TEST,
            "copy-label": "Copy link",
    ...
    test("§7.18 choosing a destination fires onShare with its id and closes", async () => {
        const onShare = vi.fn();
        const seen: ShareChooserShareDetail[] = [];
        document.body.addEventListener("share", (e) => {
            seen.push((e as CustomEvent<ShareChooserShareDetail>).detail);
        });
        const el = mount({ label: "Share", url: URL_UNDER_TEST });
        el.onShare = onShare;
        await flush();
...
describe("<share-chooser> — url resolution and custom glyph (§7.20–§7.22)", () => {
    ...
    test("§7.22 renderButtonContent replaces the glyph and sees open + url", async () => {
        ...
        mount({ label: "Share", url: URL_UNDER_TEST }, { tag: "custom-share-chooser" });
        await flush();
        const custom = document.body.querySelector<HTMLElement>('[data-testid="custom"]')!;
        // The custom node replaced the glyph...
        expect(document.body.querySelector(".share-chooser-icon")).toBeNull();
...
describe("<share-chooser> — HTML custom-element surface", () => {
    ...
    test("the class attribute lands on the rendered root, not just the host", async () => {
        ...
        await flush();
        const root = document.body.querySelector("div.share-chooser")!;
        expect(root.className).toBe("share-chooser my-share");
    });
