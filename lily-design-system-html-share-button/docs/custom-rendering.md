...
class SvgShareButton extends ShareButton {
  renderButtonContent() {
    const ns = "http://www.w3.org/2000/svg";
    const svg = document.createElementNS(ns, "svg");
    svg.setAttribute("class", "share-button-icon");
...
customElements.define("svg-share-button", SvgShareButton);
```
...
class LabelledShareButton extends ShareButton {
  renderButtonContent() {
    ...
    text.className = "share-button-text";
...
```js
class ChevronShareButton extends ShareButton {
  renderButtonContent() {
...
```js
class UrlShareButton extends ShareButton {
  renderButtonContent() {
    const span = document.createElement("span");
    span.setAttribute("aria-hidden", "true");
...
```js
if (!customElements.get("svg-share-button")) {
  customElements.define("svg-share-button", SvgShareButton);
}
```

The guard keeps hot-reload and re-imports from throwing. Import the
class from `.../share-button` (not the barrel) if you want the subclass
without also registering the base `<share-button>` tag.
