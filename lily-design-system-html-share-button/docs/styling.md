...
| Hook | Element |
| ---- | ------- |
| `.share-button` | Rendered root `<div>`. Your `class` attribute is appended here. |
| `.share-button-trigger` | The `<button>`. Note: `-trigger`, not `-button`. |
| `.share-button-icon` | The `aria-hidden` glyph `<span>`. |
| `.share-button-list` | The `<ul>`. Carries `hidden` when closed. |
| `.share-button-list-item` | Each `<li>`. |
| `.share-button-target` | Each destination `<a>`. Also `[data-target-id]`. |
| `.share-button-copy` | The copy `<button>`, when `copy-label` is set. |
| `.share-button-status` | The polite live region `<p>`. |

The custom element itself (`share-button`) is an unstyled inline element
...
this with `locale-select`, which sets `dir` on the document root.
