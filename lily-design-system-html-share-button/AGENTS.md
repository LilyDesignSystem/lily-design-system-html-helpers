# AGENTS — `<share-button>` (HTML helper)
...
`<share-button>` custom element. A single-glyph button (↪, U+21AA) that
...
| `share-button.ts` | Implementation (TypeScript custom-element class). |
| `share-button.test.ts` | Vitest + jsdom spec, mapped to the §7 clauses. |
...
- Class `ShareButton extends HTMLElement` (registered as
  `<share-button>` on import of `index.ts`).
- Named exports: `ShareButton`, `canShareNatively`, `canCopy`,
  `nextShareButtonId`, `RIGHTWARDS_ARROW_WITH_HOOK`.
- Type exports: `ShareButtonProps`, `ShareButtonShareDetail`,
  `ShareButtonUrlDetail`, `ShareTarget`, `ShareStrategy`.
...
2. **`targets` and the three callbacks are property-only.**
   ...
   matching how `theme-select` exposes `themechange`. Callback fires
...
document and nothing is persisted — unlike the `*-select` helpers, this
owns an action, not a preference.

## HTML

`<share-button>` contains one rendered
`<div class="share-button {class}">` holding, in order: a
...
`<span class="share-button-icon" aria-hidden="true">↪</span>`; a
`<ul class="share-button-list" id="{listId}" hidden>` of
`<li class="share-button-list-item">` containing
`<a class="share-button-target" data-target-id target="_blank"
rel="noopener noreferrer">` and an optional
`<button class="share-button-copy">`; and a
`<p class="share-button-status" aria-live="polite">`.

**Not a menu, and not this catalog's listbox.** The three `*-select`
...
The trigger class is `share-button-trigger`, not `-button`, because
