# `<share-button>` — Specification

Single source of truth for the `lily-design-system-html-share-button`
...
- `share-button.ts` — the implementation (custom-element class)
- `share-button.test.ts` — vitest + jsdom spec exercising every clause in §7
...
- **Disclosure + real links, not a menu, and not the catalog's
  listbox.** The three `*-select` helpers in this catalog render an icon
...
contract**, matching how `theme-select` exposes `themechange` and
`locale-select` exposes `localechange`. Consumers who never touch JS
...
2. **The trigger class is `share-button-trigger`**, not
   `share-button-button` — a deliberate exception, since
   `.share-button-button` reads badly. It is the one bend in the
   catalog's `{helper}-button` convention.
3. **`target="_blank" rel="noopener noreferrer"`** on destinations
   unless the target sets `newTab: false`. `rel` is kept either way.

There is no hidden `<input>`: unlike the `*-select` helpers this control
...
`index.ts` exports `ShareButton`, `canShareNatively`, `canCopy`,
`nextShareButtonId`, `RIGHTWARDS_ARROW_WITH_HOOK`, and the types
`ShareButtonProps`, `ShareButtonShareDetail`, `ShareButtonUrlDetail`,
`ShareTarget`, `ShareStrategy`. Importing it registers `<share-button>`.
...
simplification against `theme-select`.
...
`share-button.test.ts` asserts every clause below. Test names are
...
- Package: lily-design-system-html-share-button
- Version: 0.1.0
- License: MIT
