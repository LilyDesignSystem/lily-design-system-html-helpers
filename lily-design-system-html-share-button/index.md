# `<share-button>` — Lily Design System HTML helper
...
```sh
npm install lily-design-system-html-share-button
```
...
<script type="module">
  import "lily-design-system-html-share-button";
...
Importing the module registers `<share-button>`. Registration is
idempotent. To control the tag name yourself, import the class from
`lily-design-system-html-share-button/share-button` instead and call
...
`title` is a global HTML attribute: `<share-button title="…">` would
...
and each callback has a **matching event** — the same way `theme-select`
exposes `themechange` and `locale-select` exposes `localechange`.
...
<share-button label="Share">
  <div class="share-button">
    <button type="button" class="share-button-trigger"
            aria-label="Share" aria-expanded="false" aria-controls="share-button-1-list">
      <span class="share-button-icon" aria-hidden="true">↪</span>
    </button>
    <ul class="share-button-list" id="share-button-1-list" hidden>
      <li class="share-button-list-item">
        <a class="share-button-target" data-target-id="mastodon"
           href="…" target="_blank" rel="noopener noreferrer">Mastodon</a>
      </li>
      <li class="share-button-list-item">
        <button type="button" class="share-button-copy">Copy link</button>
      </li>
    </ul>
...
- **The trigger's class is `share-button-trigger`**, not
  ...
  which here would read `.share-button-button`.
- **Destinations are real links with no `role` override.** They are
  ...
  disclosure rather than the listbox the `*-select` helpers use.

## Styling

The package ships no CSS — including the list's positioning. The
`<ul class="share-button-list">` renders in normal flow until you give
...
.share-button-target,
.share-button-copy { display: block; inline-size: 100%; text-align: start; }
```
...
```js
import { ShareButton } from "lily-design-system-html-share-button/share-button";

class LabelledShareButton extends ShareButton {
  renderButtonContent() {
...
customElements.define("labelled-share-button", LabelledShareButton);
```
...
- [`lily-design-system-html-theme-select`](../lily-design-system-html-theme-select/)
- [`lily-design-system-html-locale-select`](../lily-design-system-html-locale-select/)
- [`lily-design-system-html-text-size-select`](../lily-design-system-html-text-size-select/)
- [Svelte original](../../lily-design-system-svelte-helpers/lily-design-system-svelte-share-button/)
