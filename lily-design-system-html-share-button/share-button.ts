...
export type ShareButtonShareDetail = {
...
export type ShareButtonUrlDetail = {
...
/** Custom-element class implementing `<share-button>`. */
export class ShareButton extends HTMLElement {
    ...
    readonly #baseId = nextShareButtonId();
    ...
    /** id of the rendered `<ul class="share-button-list">`. */
    get listId(): string {
    ...
    async #shareNatively(): Promise<boolean> {
        ...
        try {
            ...
            this.dispatchEvent(
                new CustomEvent<ShareButtonUrlDetail>("nativeshare", {
                    detail: { url: shareUrl },
                    bubbles: true,
                    composed: true,
                }),
    ...
    #chooseTarget(target: ShareTarget): void {
        ...
        this.dispatchEvent(
            new CustomEvent<ShareButtonShareDetail>("share", {
    ...
    async #copyUrl(): Promise<void> {
        ...
        try {
            ...
            this.dispatchEvent(
                new CustomEvent<ShareButtonUrlDetail>("copy", {
                    detail: { url: shareUrl },
                    bubbles: true,
                    composed: true,
                }),
    ...
    #render(): void {
        ...
        root.className = `share-button ${extraClass}`.trim();
        root.addEventListener("focusout", this.#onRootFocusOut);

        const button = document.createElement("button");
        button.type = "button";
        // Deliberately `share-button-trigger`, not `share-button-button`:
        // the sibling helpers use `{helper}-button`, which here would read
        // `.share-button-button`. The one bend in the convention.
        button.className = "share-button-trigger";
        ...
        list.className = "share-button-list";
        list.id = this.listId;
        ...
        if (this.copyLabel) {
            ...
            copyEl.type = "button";
            copyEl.className = "share-button-copy";
        ...
        status.className = "share-button-status";
