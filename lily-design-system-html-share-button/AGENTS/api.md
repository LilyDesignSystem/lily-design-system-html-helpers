...
matching `theme-select`'s `themechange` and `locale-select`'s
`localechange`:

| Event | Detail | Callback |
| ----- | ------ | -------- |
| `share` | `ShareButtonShareDetail` = `{ targetId, url }` | `onShare(targetId, url)` |
| `copy` | `ShareButtonUrlDetail` = `{ url }` | `onCopy(url)` |
| `nativeshare` | `ShareButtonUrlDetail` = `{ url }` | `onNativeShare(url)` |
...
| `items()` | `.share-button-target, .share-button-copy` in DOM order. |
...
From `share-button.ts` and re-exported by `index.ts`:

- `ShareButton` (class)
- `canShareNatively()`, `canCopy()` — SSR-safe capability probes
- `nextShareButtonId()` — module-counter id minter
- `RIGHTWARDS_ARROW_WITH_HOOK` — the `"↪"` glyph constant
- types `ShareButtonProps`, `ShareButtonShareDetail`,
  `ShareButtonUrlDetail`, `ShareTarget`, `ShareStrategy`

`index.ts` additionally registers `<share-button>` as a side effect,
