# SSR — `<share-button>` (HTML helper)

Catalog-wide rules: [`../../AGENTS/ssr.md`](../../AGENTS/ssr.md).

## This helper is the easy one

`theme-select` has a genuine SSR problem: the theme must be applied
before first paint or the user sees a flash of the wrong theme, which is
why it ships a cookie + inline-script recipe. `locale-select` and
`text-size-select` have smaller versions of the same problem.

`<share-button>` has **none of it**. It is an action, not a preference:
...
`nextShareButtonId()` uses a module counter, not `Math.random()` or
...
<script type="module">
  ...
  for (const el of document.querySelectorAll("share-button")) {
...
```css
share-button { display: inline-block; min-inline-size: 2.5rem; min-block-size: 2.5rem; }
```
