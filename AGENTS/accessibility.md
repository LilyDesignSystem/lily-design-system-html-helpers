...
| ------ | --------- | ----- | ------------------- |
| `<theme-picker>` | Icon button + `role="listbox"` dropdown | `◑` U+25D1 | The element's own JS handlers |
| `<locale-picker>` | Icon button + `role="listbox"` dropdown | U+1F310 + VS15 | The element's own JS handlers |
| `<text-size-picker>` | Icon button + `role="listbox"` dropdown | `A` U+0041 | The element's own JS handlers |

`<text-size-picker>` was deliberately left as a native `<select>` when
...
`<theme-picker>` host. The host element has no role and is silent.
...
This now applies to all three helpers, `<text-size-picker>` included.
...
- **Set `role` on the custom element itself.** The roles belong on
  ...
  `role="listbox"` on `<theme-picker>` produces a duplicate
  announcement.
