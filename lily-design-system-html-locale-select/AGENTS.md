...
consumer styles the `locale-select` class hooks on rendered children.
...
| `locale-select.test.ts`    | Vitest + jsdom spec, one test per §7 clause.     |
...
- Named exports: `LocaleSelect`, `bcp47LocaleTag`, `isRtlLocale`,
  ...
  `RTL_LANGUAGE_TAGS`, `RTL_SCRIPT_SUBTAGS`, `nextLocaleSelectId`,
...
`<locale-select>` renders a `<div class="locale-select {class}">`
containing, in order: a `<input type="hidden" name="{name}">` for
form participation; a `<button type="button" class="locale-select-button"
...
`nextLocaleSelectId()`, a module-level counter (SSR-safe; no
