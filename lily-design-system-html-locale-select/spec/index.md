...
6. Ships zero CSS — the consumer styles every visual aspect via the
   `locale-select` class hooks and the `lang` / `dir` attributes.
...
- **Stable, SSR-safe ids.** List and option ids come from a
  module-level incrementing counter (`nextLocaleSelectId()`, exported)
...
- The rendered root is a `<div>` carrying the `locale-select` class
...
- The default button glyph is **U+1F310 GLOBE WITH MERIDIANS**
  ...
  rather than as a blue colour emoji, matching theme-select's ◑
  (U+25D1, which needs no selector). It is wrapped in
  `<span class="locale-select-icon" aria-hidden="true">` so it can
...
- Each `<li>` carries `lang="{tagFor(locale)}"` (BCP 47 hyphen form)
  ...
  not locale-specific.
- Option and list ids come from `nextLocaleSelectId()`, so multiple
...
- `nextLocaleSelectId` (the id counter)
...
`<span class="locale-select-icon">`. `this.value`, `this.open`, and
...
1. The rendered root is a `<div class="locale-select">` containing a
   ...
   `<span class="locale-select-icon" aria-hidden="true">` carrying
...
23. Setting `el.localeLabels` as an object property mirrors the JSON
    ...
    the root class hook (`class="locale-select my-picker"`); and
...
29. A subclass overriding `renderButtonContent()` replaces the
    default glyph — the `locale-select-icon` span is gone and the
    returned node sits inside `button.locale-select-button` — while
