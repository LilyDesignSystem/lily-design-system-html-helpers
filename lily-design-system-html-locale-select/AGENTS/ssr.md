...
in browsers. The SSG / server emits the literal `<locale-select>`
...
<html lang={locale} dir={RTL ? "rtl" : "ltr"}>
    <head>
        <script type="module" src="/scripts/locale-select.js"></script>
    </head>
    <body>
        <locale-select
            label="Language"
...
```ts
document.querySelector("locale-select")?.addEventListener("localechange", async (e) => {
...
<script type="module">
    ...
    document.querySelector("locale-select")?.addEventListener("localechange", (e) => {
        const { locale } = (e as CustomEvent<{ locale: string }>).detail;
...
`<locale-select value="…">` attribute differs per request.
...
`<locale-select>` host arrives as a plain tag; the browser invokes
