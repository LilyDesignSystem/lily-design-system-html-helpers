...
is registered (`import "lily-design-system-html-locale-select"`) and
that you have applied at least the positioning CSS from
[styling.md](./styling.md).

Throughout, `el` is `document.querySelector("locale-select") as LocaleSelect`.
...
class CodeLocaleSelect extends LocaleSelect {
  renderButtonContent(): Node {
    const span = document.createElement("span");
    span.className = "locale-select-code";
    span.setAttribute("aria-hidden", "true");
    span.textContent = this.value.split(/[-_]/)[0].toUpperCase();  // "fr_CA" → "FR"
    return span;
  }
}
customElements.define("code-locale-select", CodeLocaleSelect);
```
...
```ts
import { isRtlLocale } from "lily-design-system-html-locale-select";
router.afterEach(() => {
