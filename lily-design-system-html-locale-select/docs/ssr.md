...
`<locale-select>` is a custom element. The class only runs in the
browser; there is no "select on the server". The static-site
generator (or server-rendered HTML pipeline) emits the literal
`<locale-select>` tag with attributes; the browser constructs the
...
- `value="{code}"` on the `<locale-select>` host.
...
└── _config/
    └── locale-select.js      ← writes the helper to /dist/
```
...
2. Parses `<locale-select value="fr">` — the select constructor
...
| ~50 ms   | `<locale-select>` tag parsed. Element is "stub" HTMLElement. |
