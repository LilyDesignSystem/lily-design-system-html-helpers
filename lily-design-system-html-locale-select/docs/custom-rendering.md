...
default `<span class="locale-select-icon">`. The three values the
...
```ts
class LabelledLocaleSelect extends LocaleSelect {
    renderButtonContent(): Node {
        const span = document.createElement("span");
        span.className = "locale-select-button-label";
        span.lang = this.tagFor(this.value);
        span.textContent = this.labelFor(this.value);
        return span;
    }
}
customElements.define("labelled-locale-select", LabelledLocaleSelect);
```
...
`.locale-select-button[aria-expanded="true"]` over re-rendering on
...
`.locale-select-button[aria-expanded="true"]` in CSS, which the base
...
```ts
class TextLocaleSelect extends LocaleSelect {
    renderButtonContent(): Node {
        ...
        text.className = "locale-select-button-text";
...
```ts
class SvgLocaleSelect extends LocaleSelect {
    renderButtonContent(): Node {
...
customElements.define("svg-locale-select", SvgLocaleSelect);
```
...
```ts
class ButtonRowLocaleSelect extends LocaleSelect {
    ...
    #rebuild(): void {
        const root = this.querySelector<HTMLElement>("div.locale-select");
        if (!root) return;

        // Hide the base control but keep the root (class hook) and the
        // hidden input (form participation).
        root.querySelector<HTMLElement>(".locale-select-button")!.hidden = true;
        root.querySelector(".locale-select-buttons")?.remove();

        const row = document.createElement("div");
        row.className = "locale-select-buttons";
...
customElements.define("button-row-locale-select", ButtonRowLocaleSelect);
```
...
1. **Keep the `locale-select` class hook** on whatever root you
...
it("renders custom button content and keeps the aria wiring", () => {
    ...
    const btn = el.querySelector(".locale-select-button")!;
