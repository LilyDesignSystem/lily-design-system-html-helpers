...
function mount(attrs: Record<string, string>): MotionPicker {
  const el = document.createElement("motion-picker") as MotionPicker;
...
describe("<motion-picker> — pure helpers", () => {
...
describe("<motion-picker> — markup contract (§7.1–§7.5)", () => {
  ...
  test("§7.1 the button renders the default pause-bars SVG icon, hidden from assistive tech", async () => {
    mount({ label: "Motion", motions: MOTIONS.join(",") });
    await flush();
...
describe("<motion-picker> — application (§7.6–§7.11)", () => {
...
describe("<motion-picker> — element shape + property API (§7.12–§7.13)", () => {
...
describe("<motion-picker> — keyboard contract (APG listbox, §7.14–§7.18)", () => {
...
describe("<motion-picker> — custom rendering by subclass (§7.19)", () => {
...
describe("<motion-picker> — accessibility hardening (§7.20–§7.23; canonical Svelte §7.14–§7.17)", () => {
...
describe("pointer open survives the button content swap (regression)", () => {
  ...
  test("clicking the icon span opens and STAYS open", async () => {
    const el = document.createElement("motion-picker");
    el.setAttribute("label", "Motion");
