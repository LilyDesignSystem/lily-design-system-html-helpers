...
aria-controls>` whose only content is the `aria-hidden` bundled
two-bar-pause SVG icon (not a Unicode character — reversed
2026-09-16; replaceable via `renderButtonContent()`), and a
...
idempotent-apply rule, same bundled-SVG-icon convention. It is additive:
