...
| ------------------------------------------ | --------------------------------------------- | ------------- |
| `<text-size-select>` (host)                | none (transparent)                            | —             |
| `<div class="text-size-select">`           | none (styling root)                           | Element       |
| `<button class="text-size-select-button">` | implicit `role="button"`                      | Browser       |
...
| `<span class="text-size-select-icon">`     | `aria-hidden="true"`                          | Element       |
| `<ul class="text-size-select-list">`       | `role="listbox"`, `aria-label={label}`        | Element       |
| `<ul>`                                     | `tabindex="-1"`; `hidden` while closed        | Element       |
| `<ul>`                                     | `aria-activedescendant={optionId}` while open | Element       |
| `<li class="text-size-select-option">`     | `role="option"`, unique `id`, `aria-selected` | Element       |
| `<li>`                                     | `data-active` when keyboard-highlighted       | Element       |
| `<input type="hidden">`                    | none — form participation only                | Element       |

The host `<text-size-select>` element has no implicit ARIA role. It is
...
- Style `.text-size-select-list:focus-visible`, not just the button —
  that is where the focus ring belongs while the list is open.
- Give `.text-size-select-option[data-active]` a clearly visible
...
`theme-select` and `locale-select` siblings. It costs three things,
...
means *decrease* rather than *size*) or theme-select's ◑ can.
...
```html
<text-size-select label="Text size" sizes="small,medium,large,x-large"></text-size-select>

<p class="text-size-select-status" aria-live="polite">Text size: Medium</p>

<script type="module">
    await customElements.whenDefined("text-size-select");
...
- **Styling `[data-active]` and `[aria-selected]` identically.** They
  ...
  difference.
- **Forgetting `.text-size-select-list:focus-visible`.** While the
...
If you subclass `TextSizeSelect`, which tier you pick decides who owns
accessibility:
