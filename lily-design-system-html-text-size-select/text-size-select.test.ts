...
function button(): HTMLButtonElement {
    return document.body.querySelector<HTMLButtonElement>(".text-size-select-button")!;
}

function list(): HTMLUListElement {
    return document.body.querySelector<HTMLUListElement>(".text-size-select-list")!;
}

function options(): HTMLLIElement[] {
    return [...document.body.querySelectorAll<HTMLLIElement>(".text-size-select-option")];
}
...
describe("<text-size-select> — pure helpers", () => {
    // sizeName is the exported mirror of theme-select's themeName.
...
describe("<text-size-select> — markup contract (§7.1–§7.5)", () => {
    test("§7.1 renders a div root containing a button that controls a listbox", async () => {
        ...
        await flush();
        const root = document.body.querySelector("div.text-size-select")!;
    ...
    test("§7.1 the button renders the 'A' glyph, hidden from assistive tech", async () => {
        mount({ label: "Text size", sizes: SIZES.join(",") });
        await flush();
        const icon = document.body.querySelector<HTMLElement>(".text-size-select-icon")!;
...
describe("<text-size-select> — application (§7.6–§7.11)", () => {
...
describe("<text-size-select> — element shape + property API (§7.12–§7.13)", () => {
    test("§7.12 element survives a re-render with its id/data-* intact", async () => {
        ...
        expect(el.querySelectorAll(".text-size-select-option").length).toBe(2);
    });
    ...
    test("§7.12 the consumer class is appended to the root class hook", async () => {
        ...
        await flush();
        const root = document.body.querySelector("div.text-size-select")!;
        expect(root.className).toBe("text-size-select my-sizer");
    });
    ...
    test("§7.13 setting el.sizes as an array mirrors the CSV attribute and re-renders", async () => {
        ...
        expect(el.querySelectorAll(".text-size-select-option").length).toBe(3);
    });
    ...
    test("§7.13 list and option ids are unique across instances", async () => {
        ...
        const listIds = [...document.body.querySelectorAll(".text-size-select-list")].map(
            (l) => l.id,
        );
...
describe("<text-size-select> — keyboard contract (APG listbox, §7.14–§7.18)", () => {
...
describe("<text-size-select> — custom rendering by subclass (§7.19)", () => {
    class GlyphlessTextSizeSelect extends TextSizeSelect {
    ...
    if (!customElements.get("glyphless-text-size-select")) {
        customElements.define("glyphless-text-size-select", GlyphlessTextSizeSelect);
    }

    test("§7.19 renderButtonContent replaces the glyph and keeps the aria wiring", async () => {
        const el = document.createElement(
            "glyphless-text-size-select",
        ) as GlyphlessTextSizeSelect;
        el.setAttribute("label", "Text size");
        el.setAttribute("sizes", SIZES.join(","));
        ...
        expect(custom.closest("button")?.className).toBe("text-size-select-button");
        expect(el.querySelector(".text-size-select-icon")).toBeNull();
        ...
        const btn = el.querySelector<HTMLButtonElement>(".text-size-select-button")!;
        expect(btn.getAttribute("aria-haspopup")).toBe("listbox");
