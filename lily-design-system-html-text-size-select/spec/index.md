...
The control is structurally identical to its `theme-select` and
`locale-select` siblings in this catalog: an icon button that opens a
...
- **Detecting a preferred size.** There is no OS "preferred text size"
  ...
  theme-select's `detect-from-system` and locale-select's
...
- **Side-effectful registration on import.** `index.ts` calls
  ...
  `text-size-select.ts` (which does not register).
- **Light DOM.** The element renders its `<div>` root, button, and
  listbox as children, not in a shadow root. Consumer CSS targets the
  kebab-case class hooks (`text-size-select`,
  `text-size-select-button`, `text-size-select-icon`,
  `text-size-select-list`, `text-size-select-option`) directly.
...
- **Stable ids from a module counter.** `nextTextSizeSelectId()`
...
| `class`          | `string`        | no       | `""`                          | Extra CSS class on the rendered root `<div>` (in addition to `text-size-select`). |
...
<text-size-select label="Text size" sizes="small,medium,large,x-large">
  <div class="text-size-select {consumer class}">
    ...
    <button type="button" class="text-size-select-button"
            ...
            aria-expanded="false" aria-controls="text-size-select-1-list">
      <span class="text-size-select-icon" aria-hidden="true">A</span>
    </button>
    <ul class="text-size-select-list" id="text-size-select-1-list"
...
- **Root.** A `<div class="text-size-select {class}">` in light DOM.
  The consumer's `class` attribute on the host is mirrored onto it
  after the base hook.
- **Glyph.** The default button content is
  `<span class="text-size-select-icon" aria-hidden="true">` containing
...
- **`data-active` vs `aria-selected`.** `data-active` marks the
  ...
  open.
- **ids** come from `nextTextSizeSelectId()`, a module-level
...
- `sizeName` (slug → title-cased label; the mirror of theme-select's
  `themeName` and locale-select's `localeName`. `labelFor` delegates
  to it, so there is exactly one implementation of the rule)
- `nextTextSizeSelectId` (the id counter)
...
- Whatever `Node` it returns is placed inside the button, replacing
  the default `<span class="text-size-select-icon">`.
...
| ---------------------------------- | --------------- |
| `<text-size-select>` (host)        | none — a transparent lifecycle container. |
| `<div class="text-size-select">`   | none — a styling root. |
| `<button>`                         | implicit `button` role; `aria-label={label}`, `aria-haspopup="listbox"`, `aria-expanded`, `aria-controls={listId}`. |
| `<span class="text-size-select-icon">` | `aria-hidden="true"` so the glyph never becomes the name. |
...
theme-select and locale-select. All three are the consumer's to
mitigate.
...
`text-size-select.test.ts` must assert every numbered item below, and
...
1. The rendered root is a `<div class="text-size-select">` containing a
   ...
   `<span class="text-size-select-icon" aria-hidden="true">` holding
...
2. `aria-label` carries the supplied `label` on **both** the button and
   the listbox.
3. One `<li class="text-size-select-option">` is rendered per entry in
...
12. Extra DOM properties / attributes on the host survive re-rendering
    ...
    `class="text-size-select my-sizer"`.
...
19. A subclass overriding `renderButtonContent()` replaces the default
    glyph — no `.text-size-select-icon` is rendered — while the
